...
import { HOSPITAL_SERVICE_MAP, HOSPITAL_TYPES } from '../constants/serviceConstants';
...
export const HospitalForm: React.FC<HospitalFormProps> = ({
  ...
  const getCurrentServices = () => {
    const typeKey = formData.hospitalType;
    return typeKey ? (HOSPITAL_SERVICE_MAP[typeKey] || []) : [];
  };
                ...
                startAdornment: (
                  <InputAdornment position="start">
                    {validationState.checkingName ? (
                      ...
                      <LocalHospital sx={{ color: validationState.nameExists ? 'error.main' : 'inherit' }} />
                    )}
                  </InputAdornment>
                ),
              }}
          ...
          {formData.hospitalType && (
            <Grid item xs={12}>
              <Typography variant="subtitle1" gutterBottom>
                Available Services for {formData.hospitalType}
              </Typography>
